//Tran Vu Hong Phuc - 21ES - Exericses3
//Library use for this exercises
//server  
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#define PORT 8080 //Define the port number of sever
#define SHMSIZE 2000 //Define the size of shared memory
#define SHMKEY  4000 //Define a key for shared memory access

// Shared resource
char *shared_data; //Pointer to the shared memory
sem_t *semaphore; /*Using semaphore to sever handle multiple client connect. Without this could lead to race conditions */
sem_t client_semaphore;
int shared_data_updated = 0; 
int client_count = 0; //Count for the number of user connect to sever.
...
void *log_shared_memory(void *arg) {
    ...
    while (1) {
        ...
        if (shared_data_updated) {
            char data[SHMSIZE]; //Buffer to hold shared data memory
            strncpy(data, shared_data, SHMSIZE); //Copy shared data to local buffer
            shared_data_updated = 0; //Reset the update flag
            sem_post(semaphore); //Leave the semaphore
        } else {
            sem_post(semaphore);
...
void *handle_client(void *arg) {
    int client_socket = *(int *)arg; //Retrieve the client socket from the argument
    free(arg); //Free the allocated shared memory
    ...
    while (1) {
        ...
        sem_wait(semaphore);
        snprintf(shared_data, SHMSIZE, "Client %d: %s", client_number, buffer);
        ...
        sem_wait(semaphore);
        snprintf(shared_data, SHMSIZE, "Server to Client %d: %s", client_number, response);
        shared_data_updated = 1;
        sem_post(semaphore);
...
int main() {
    ...
    int shm_id = shmget(SHMKEY, SHMSIZE, IPC_CREAT | 0666);
    if (shm_id == -1) {
        perror("shmget");
        exit(EXIT_FAILURE);
    }
